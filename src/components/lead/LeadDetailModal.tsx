...
📍Ci troviamo in Via Giacomo Leopardi 7 Pedrengo (BG)`
  ...
  yoga_trial: {
    label: 'Yoga - Classe di Prova',
    template: `🧘‍♀️ Buongiorno [nome_lead]! Grazie per l'interesse verso le nostre lezioni di Yoga.
In merito alla sua richiesta, le confermo i dettagli per la lezione di prova di [tipo di tagliando]:
💰 Prezzo speciale: [prezzo] per il pass d'ingresso.
📅 Ti andrebbe di prenotare subito una sessione di prova gratuita dal nostro calendario dell'app?
Restiamo a disposizione per qualsiasi chiarimento sugli orari delle nostre classi.
Un cordiale saluto!`
  },
  web_site_quote: {
    label: 'Siti Web - Info Preventivo',
    template: `💻 Gentile [nome_lead], grazie per averci contattato per la tua richiesta di creazione sito web / [tipo di tagliando].
In allegato o di seguito, trovi una stima informativa dei costi per il tuo progetto:
💰 Budget indicativo: [prezzo] (IVA compl.)
Cosa include la nostra soluzione:
• Design responsive ottimizzato per smartphone e tablet
• SEO di base per posizionamento locale su Google
• Modulo contatti dinamico integrato con Lead Hub
📅 Possiamo fissare una chiamata di fattibilità per approfondire il tuo progetto?
Cordiali saluti!`
  },
  generic_contact: {
    label: 'Contatto Generico Professional',
    template: `👋 Salve [nome_lead], grazie per la tua richiesta relativa a [tipo di tagliando]!
Siamo felici di darti tutte le informazioni necessarie. In merito al costo indicativo del servizio richiesto:
💰 Prezzo stimato: [prezzo]
📅 Quando saresti disponibile per una breve videochiamata conoscitiva o per fissare un appuntamento in sede?
...
const LeadDetailModal: React.FC<LeadDetailModalProps> = ({ isOpen, onClose, lead, client, historicalLeads, onAddNote, onUpdateNote, onDeleteNote, onHistoricalLeadAdded, onHistoricalLeadUpdated, onHistoricalLeadDeleted, onLeadUpdate }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            const otherServices = (client?.services || []).filter(s => s.name !== '__default_fields__');
            if (otherServices[0]) {
                setHistoricalService(otherServices[0].name);
            } else {
    ...
    const isFacchetti = true; // Abilitiamo Preventivi, Appuntamenti e Modelli Veloci per TUTTI i nostri clienti!
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center space-x-2">
                                        ...
                                        <a 
                                            ...
                                            className="p-2 hover:bg-blue-500/10 rounded-md"
                                        >
