...
const MailCampaignModal: React.FC<Props> = ({ isOpen, onClose, campaign, client, templates, canSend, onSaved }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex flex-col bg-slate-50 dark:bg-slate-900">
            {/* ── Top bar ── */}
            <div className="shrink-0 bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border-b border-slate-200/70 dark:border-slate-700/60">
                    ...
                    <div className="ml-auto flex items-center gap-2">
                        ...
                        {scheduleMode === 'scheduled' && (
                            <button onClick={handleSchedule} disabled={isSaving || isSending || !scheduledAt}
                                className="flex items-center gap-1.5 px-4 py-1.5 bg-amber-500 hover:bg-amber-600 disabled:opacity-50 disabled:cursor-not-allowed text-white text-xs font-semibold rounded-lg transition-colors">
                                {isSaving ? <Loader2 size={13} className="animate-spin" /> : <Calendar size={13} />}
                                {!scheduledAt ? 'Seleziona data ↓' : 'Pianifica invio'}
                            </button>
                        )}
                        {scheduleMode === 'now' && canSend && (
                ...
                {/* Banner errore ben visibile */}
                {error && (
                    <div className="px-4 py-2 bg-red-50 dark:bg-red-900/20 border-t border-red-200 dark:border-red-800/40 text-sm text-red-600 dark:text-red-400 font-medium">
                        ⚠ {error}
                    </div>
                )}
            </div>
